...
bool MyController1::onGet(const HttpRequest &req, HttpResponse &res, const HttpConfig &) {
    ...
    json::Json retJson(
        {{"ret", 200}, {"errmsg", ""}, {"data", "data"}, {"postdata", req.getPostParams()}, {"query", req.getQuery()}});

    res.setBody(retJson.dump());
    return true;
}

bool MyController1::onPost(const HttpRequest &, HttpResponse &res, const HttpConfig &cfg) { return true; }

bool MyController1::onPut(const HttpRequest &, HttpResponse &res, const HttpConfig &cfg) { return true; }
