...
std::string _ltrim(cstring src, char ch) {
    std::string           temp = src;
...
void HttpUrl::resetUrl(cstring url) {
    fullurl = url;
    parse();
}

std::string HttpUrl::getHostUrl() const {
    std::stringstream ss;
    ss << scheme << "://";
    if (!username.empty())
        ss << username;
...
HttpRequest::HttpRequest(const ResourceMap &headerMap)
    : m_vReqestHeader(headerMap) {
}

void HttpRequest::setRequestType(cstring reqType) {
    m_strRequestType = reqType;
}

void HttpRequest::setRequestPath(cstring reqPath) {
...
void HttpRequest::setHttpVersion(cstring httpversion) {
    m_strRequestHttpVersion = httpversion;
}

void HttpRequest::setParams(cstring params) {
...
ostream &operator<<(ostream &os, HttpRequest &obj) {
...
ResourceMap HttpRequest::getHeader() const {
...
void HttpResponse::setBody(cstring key, cstring val) {
    if (val.empty())
        m_vResponseBody.back().second += key;
    else
        m_vResponseBody.push_back(std::pair<std::string, std::string>(key, val));
}
std::string HttpResponse::getResponseText() const {
    return m_strResponseText;
}
void HttpResponse::getResponseBytes(std::ostream &os) {
...
size_t HttpResponse::getResponseBytesSize() const {
    return m_nBodyBytes;
}
size_t HttpResponse::getResponseDecodeBytesSize() const {
    return m_nBodyDecodeBytes;
}
std::string HttpResponse::getResponseItem(cstring key) {
...
std::string HttpResponse::getCookie() {
...
bool HttpResponse::BodyIsDecoded() const {
    return m_bDecodeBodyStatus;
}
const ResourceMap &HttpResponse::GetResponse() const {
    return m_vResponseBody;
}
ostream &operator<<(ostream &os, HttpResponse &obj) {
...
void SocketClient::printSSLConnection(SSL *ssl) {
...
void SocketClient::SSLDisConnect() {
    if (m_pConnection) {
...
ssize_t SocketClient::SSLRecv(int fd, void *buf, ssize_t size) {
    return SSL_read(m_pConnection->m_ptrHandle, buf, size);
}

ssize_t SocketClient::SSLSend(const void *buf, ssize_t size) {
...
int SocketClient::RecvChunkData(const char *buffer, size_t &nPos, int size, HttpResponse &Response, CHUNK_STATE &ChunkState, size_t &ChunkSize, ssize_t &AllChunkBodySize, size_t &blockCount) {
...
Encoding SocketClient::ParseHeader(const char *buffer, ssize_t size, HttpResponse &Response, ssize_t &HeaderSize, ssize_t &recvBodySize, ssize_t &LeftBodySize, ssize_t &BodySizeInResponse) {
...
void SocketClient::ParseHeaderLine(cstring line, std::string &key, std::string &val) {
    int nBlankCnt = 0;
    key.clear();
    val.clear();
...
void SocketClient::ParseFirstLine(cstring line, std::string &HttpVersion, int &StatusCode, std::string &StatusMessage) {
    int nBlankCnt = 0;
    StatusCode    = 0;
    for (size_t i = 0; i < line.size(); i++) {
        if (line[ i ] == ' ') {
...
ssize_t SocketClient::read(HttpResponse &Response, cstring outfile) {
...
std::string SocketClient::getChunkString(Encoding nType) {
    switch (nType) {
        case EncodingLength:
...
double SocketClient::getSpendTime(std::chrono::system_clock::time_point &tBegin) {
    auto end      = std::chrono::system_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end - tBegin);
    return double(duration.count()) * std::chrono::microseconds::period::num / std::chrono::microseconds::period::den;
}

double SocketClient::getCurrentSpeed(std::chrono::system_clock::time_point &tBegin, ssize_t receivedBytes, char nType) {
    auto end      = std::chrono::system_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end - tBegin);
...
ssize_t SocketClient::write(const char *writeBuf, ssize_t nSize) {
#ifndef USE_OPENSSL
...
bool SocketClient::connect(cstring url) {
    return connect(HttpUrl(url));
}

bool SocketClient::connect(const HttpUrl &url) {
...
int SocketClient::recvData(int fd, void *buf, size_t size, int ops) {
#ifndef USE_OPENSSL
...
void SocketClient::setConnectTimeout(size_t nSecond) {
    m_nConnectTimeout = nSecond;
}

int SocketClient::setFDnonBlock(int fd) {
...
std::string SocketClient::getErrorString() const {
    return m_strErrorString;
}

void SocketClient::setRequestType(cstring RequestType) {
    m_strRequestType = RequestType;
}

std::string SocketClient::getRequestType() const {
    return m_strRequestType;
}

int SocketClient::getConnectedFd() const {
    return m_nConnectFd;
}

void HttpClient::setOutputFile(cstring file) {
    m_strOutputFile = file;
}

HttpResult HttpClient::Request(cstring reqUrl, bool bRedirect, bool verbose) {
...
std::string HttpClient::GetRedirectLocation(cstring strLocation, cstring RequestPath) {
...
HttpResult HttpClient::Get(cstring reqUrl, bool bRedirect, bool verbose) {
    m_ReqHeader.setRequestType("GET");
    m_SocketClient.setRequestType("GET");
    return Request(reqUrl, bRedirect, verbose);
}

HttpResult HttpClient::Post(cstring reqUrl, cstring strParams, bool bRedirect, bool verbose) {
...
HttpResult HttpClient::Head(cstring reqUrl, bool verbose) {
    m_ReqHeader.setRequestType("HEAD");
    m_SocketClient.setRequestType("HEAD");
    return Request(reqUrl, false, verbose);
}

void HttpClient::SaveResultToFile(cstring fileName) {
...
void HttpClient::setBasicAuthUserPass(cstring user, cstring passwd) {
    std::stringstream ss;
    ss << user << ":" << passwd;
...
void HttpClient::setUserAgent(cstring AgentVal) {
    this->setHeader(UserAgent, AgentVal);
}

void HttpClient::setContentType(cstring ContentTypeVal) {
    this->setHeader(ContentType, ContentTypeVal);
}

void HttpClient::setAcceptLanguage(cstring AcceptLanguageVal) {
    this->setHeader(AcceptLanguage, AcceptLanguageVal);
}

void HttpClient::setAcceptEncoding(cstring AcceptEncodingVal) {
    this->setHeader(AcceptEncoding, AcceptEncodingVal);
}

void HttpClient::setAccept(cstring AcceptVal) {
    this->setHeader(Accept, AcceptVal);
}

void HttpClient::setHttpVersion(utils::HttpVersion version) {
...
void HttpClient::setConnectTimeout(size_t nSeconds) {
    m_SocketClient.setConnectTimeout(nSeconds);
}

void HttpClient::setCookie(cstring strCookie) {
    this->setHeader(Cookie, strCookie);
}

void HttpClient::SaveTempFile() {
...
void HttpClient::setReferer(cstring strReferer) {
    this->setHeader(Referer, strReferer);
}

std::string HttpClient::getCookie() {
    return m_Response.getCookie();
}

void HttpClient::DownloadFile(cstring reqUrl, cstring downloadPath, int nThreadCount, bool verbose) {
...
HttpClient::DownloadThreadMgr::RangeInfo::RangeInfo(ssize_t begin, ssize_t end)
    : nBegin(begin)
    , nEnd(end) {
}
std::string HttpClient::DownloadThreadMgr::RangeInfo::strRange() const {
    std::stringstream ss;
    ss << "bytes=" << nBegin << "-" << nEnd;
    return ss.str();
}
void HttpClient::DownloadThreadMgr::RangeInfo::setRequestHeader(cstring header) {
    strRequstHeader = header;
}
void HttpClient::DownloadThreadMgr::RangeInfo::setThreadTmpFile(cstring tmpFile) {
    strTmpFile = tmpFile;
}
void HttpClient::DownloadThreadMgr::RangeInfo::setReqUrl(cstring strUrl) {
    strReqUrl = strUrl;
}
HttpClient::DownloadThreadMgr::DownloadThreadMgr(ssize_t FileLength, int nThreadCount, cstring reqUrl, HttpRequest &RequestHeader, cstring strDownloadPath)
...
bool HttpClient::DownloadThreadMgr::startThread() {
