...
int main(int argc, char **argv){
    ...
    CommandParse.add<int>("timeout",'t', "set connection timeout(s)", false, 10, cmdline::range<int>(0, 30));
    CommandParse.set_program_name("httpclient");

    bool ok = CommandParse.parse(argc, argv);
    for (size_t i = 0; i < CommandParse.rest().size(); i++)
        std::cout << "- " << CommandParse.rest()[i] << std::endl;
    if(!ok || argc < 2){
        ...
        logger.setLevel(static_cast<log::detail::Level>(nLevel));
        ...
        std::cout << "\nStatus Code:" << Result.status_code() << std::endl
              ...
              << "Text :" << Result.text().size() << std::endl;
