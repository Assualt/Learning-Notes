...
public:
    ...
    friend ostream &operator<<(ostream &os, HttpResouce &obj) {
    ...
    void tryDecodeBody() {
        if (this->getResponseItem(ContentEncoding) == "gzip") {
            ...
            if (nBytes) {
                ...
                logger.info("Decode Error.. bytes:%d", nBytes);
            }
        }
    }
    void clear() {
...
private:
    ...
    int RecvChunkData(const char *buffer, size_t &nPos, int size, HttpResouce &Response, int &ChunkState, size_t &ChunkSize,
    ...
    Encoding ParseHeader(const char *buffer, ssize_t size, HttpResouce &Response, ssize_t &HeaderSize, ssize_t &recvBodySize, ssize_t &LeftBodySize,
...
public:
    ssize_t read(HttpResouce &Response) {
...
public:
    ...
    HttpResult Get(const std::string &reqUrl, bool bRedirect = false, bool verbose = false) {
        return Request(reqUrl, TYPE_GET, bRedirect, verbose);
    }
    HttpResult Post(const std::string &reqUrl, bool bRedirect = false, bool verbose = false) {
        return Request(reqUrl, TYPE_POST, bRedirect, verbose);
    }
