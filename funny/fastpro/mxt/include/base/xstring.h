...
inline void _rtrim(tstring &src, const char ch)//移除后面空格
{
	size_t _pos = src.length() - 1;
	while (src[_pos--] == ch);
	src.erase(_pos);
}
inline void _trim(tstring & src, const char ch)//移除前后空格
{
	_ltrim(src, ch);
	_rtrim(src, ch);
}
...
public:
	...
	// src:源字符串 nSize:大小 ch:填充字符 bFront:向前填充
...
public:
	...
	static void ltrim(tstring & src);//移除前方空格
	static void trim(tstring & src);//移除前后空格
	static void rtrim(tstring & src);//移除后面空格
