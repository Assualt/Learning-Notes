...
#ifndef SSP_TOOLS_RANGE_H
#define SSP_TOOLS_RANGE_H
...
public:
    ...
    typedef typename std::conditional<std::is_same<Iter, char *>::value || std::is_same<Iter, unsigned char *>::value,
            Range<const value_type *>, Range<Iter>>::type const_range_type;
...
public:
    ...
    constexpr explicit Range(Iter str)
            : begin_(str)
...
public:
    ...
    template <typename Alloc> void reset(const Range::mstring<Alloc> &str) { reset(str.data(), str.size()); }

    [[nodiscard]] constexpr size_type size() const { return size_type(end_ - begin_); }

    [[nodiscard]] constexpr size_type walk_size() const { return size_type(std::distance(begin_, end_)); }

    [[nodiscard]] constexpr bool empty() const { return begin_ == end_; }

    constexpr Iter data() const { return begin_; }

    constexpr Iter start() const { return begin_; }

    constexpr Iter begin() const { return begin_; }

    constexpr Iter end() const { return end_; }

    constexpr Iter cbegin() const { return begin_; }

    constexpr Iter cend() const { return end_; }
    ...
    bool removePrefix(const const_range_type &prefix) { return startsWith(prefix) && (begin_ += prefix.size(), true); }

    bool removePrefix(value_type prefix) { return startsWith(prefix) && (++begin_, true); }

    bool removeSuffix(const const_range_type &suffix) { return endsWith(suffix) && (end_ -= suffix.size(), true); }

    bool removeSuffix(value_type suffix) { return endsWith(suffix) && (--end_, true); }
