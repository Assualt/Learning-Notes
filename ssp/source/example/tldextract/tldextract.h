...
private:
    ...
    bool downloadRemoteUrl(const std::string &destFileName) {
        ...
        for (auto &reqUrl : suffix_list_urls) {
            ...
            if (result.IsOk()) {
                HttpClient::SaveToFile(destFileName, result);
                logger.Info("download file %s from %s success.", destFileName, reqUrl);
                return true;
            }
            logger.Info("Download Error:%d [%s], try another url ===> %s", result.GetStatusCode().first,
                        result.GetStatusCode().second, reqUrl);
        }
        return false;
    }
    ...
    bool load_org_file(const std::string &org_file) {
        ...
        while (getline(fin, line)) {
            ...
            if (utils::startswith(line, PUNYPREFIX) ||
                (util::StartsWith(line, "//") && (icann_domain_start || psl_private_domains_start))) {
